...
public class ShowPhotoPageHandler extends AbstractWebPageHandler implements WebFormHandler {
	...
	protected String doHandleGet(UserSession us, String link, Map args) {
		...
		if (!link.equals(PartUtil.SHOW_PHOTO_PAGE_NAME)) {
			photo = SoccerPhotoManager.getInstance().getPhoto(link);
		}

		PhotoManager photoManager = SoccerPhotoManager.getInstance();
	...
	protected void makeWebPageBody(UserSession us, WebPart page) {
		...
		Photo photo = SoccerPhotoManager.getInstance().getPhoto(photoId);
	...
	public String handlePost(UserSession us, Map args) {
		...
		Photo photo = SoccerPhotoManager.getInstance().getPhoto(id);
		if (photo != null) {
